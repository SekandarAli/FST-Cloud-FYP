...
import com.example.fstsignin.Vendor.Vendor_Model
import com.example.fstsignin.Vendor.showdata_adapter
import com.google.firebase.database.*

class Search_data : AppCompatActivity() {
        ...
        query.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists()){
                    ...
                    DishRecycleview.adapter = showdata_adapter(this@Search_data,DishArrayList)

                }
                else
                {
                    Toast.makeText(this@Search_data, "snapshot does not exist", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Search_data, "Something went wrong", Toast.LENGTH_SHORT).show()
            }
        })
